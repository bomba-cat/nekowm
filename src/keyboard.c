...
void neko_grab_keybinds()
{
  ...
  for (size_t i = 0; i < sizeof(keybinds) / sizeof(neko_keybind); i++)
  {
    ...
    for (xcb_keycode_t *code = keycodes; *code != 0; ++code)
    {
      ...
      {
        xcb_grab_key(connection, 1, screen->root, keybinds[i].mod | ignored_mods[j], *code,
...
void neko_read_keybinds(xcb_key_press_event_t *e)
{
  ...
  for (size_t i = 0; i < sizeof(keybinds) / sizeof(neko_keybind); i++)
  {
    if ((e->state & ~(XCB_MOD_MASK_LOCK | XCB_MOD_MASK_2)) == keybinds[i].mod &&
        (keysym == keybinds[i].key || alt_keysym == keybinds[i].key))
    {
